...
const CreateTransaction = async (req, res = response) => {
  ...
  //Rest the money of the user to the amount you want to transfer
  const amountLessTransaction = (userTransactionTo.amount =
    userTransactionTo.amount - amount);
  const amountPlusTransaction = (userGivesMoney.amount =
    userGivesMoney.amount + amount);

  //Update DB
  await User.findByIdAndUpdate(
    userGivesMoney.id,
    { ...userGivesMoney, amountPlusTransaction },
    { new: true }
  );
  await User.findByIdAndUpdate(
    userTransactionTo.id,
    { ...userTransactionTo, amountLessTransaction },
    { new: true }
  );
